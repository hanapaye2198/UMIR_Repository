    ...
    <div class="bg-white rounded-xl shadow-md p-6">
        ...
        <form method="POST" action="{{ route('collections.update', $collection->id) }}">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" value="{{ old('name', $collection->name) }}" class="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-yellow-400" required>
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="community_id" class="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-yellow-400" required>
                    @foreach($communities as $community)
                        <option value="{{ $community->id }}" {{ $collection->community_id == $community->id ? 'selected' : '' }}>
                            {{ $community->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <textarea name="description" rows="4" class="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-yellow-400">{{ old('description', $collection->description) }}</textarea>
            </div>
